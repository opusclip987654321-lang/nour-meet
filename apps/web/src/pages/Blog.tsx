...
export function Blog() {
  ...
  return <Layout><section className="page"><h1>Rencontres, amitié et vie sociale.</h1>
    ...
    {articles.length===0?<div className="empty"><Inbox size={24} aria-hidden="true"/><h2>Aucun article pour le moment</h2></div>:<div className="event-grid">{articles.map(a=><Link key={a.id} to={`/blog/${a.slug}`} className="event-card"><div className="event-art">{a.imageUrl?<img src={imgUrl(a.imageUrl)} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span className="eyebrow">{a.category}</span>}</div><div className="event-copy"><small>{a.category.toUpperCase()}</small><h3>{a.title}</h3><p>{a.excerpt}</p></div></Link>)}</div>}
  </section></Layout>;
}
...
export function ArticlePage() {
  ...
  if(notFound)return <Layout><div className="empty"><Inbox size={24} aria-hidden="true"/><h2>Article introuvable</h2></div></Layout>;
