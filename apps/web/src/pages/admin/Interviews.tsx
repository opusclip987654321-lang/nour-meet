...
export function AdminAvailability() {
  ...
  return <Layout><section className="admin-page"><AdminNav/><div className="admin-main"><h1>Agenda des entretiens</h1><p className="fine">Un seul agenda pour toute la plateforme : un seul entretien possible à la fois.</p>{notice&&<Notice kind={notice.kind}>{notice.text}</Notice>}
    <div className="availability-layout">
      ...
      <div className="panel availability-list">
        ...
        :slots.length===0?<div className="empty small"><Inbox size={24} aria-hidden="true"/><p>Aucun créneau créé.</p></div>
        :<div className="stack">{[...byDay.entries()].map(([day,daySlots])=><div key={day} className="availability-day"><small>{new Date(day).toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long"}).toUpperCase()}</small><div className="slot-chip-grid">{daySlots.map(s=><div key={s.id} className={`slot-chip ${s.application?"booked":""}`}><span>{timeLabel(s.startsAt)}</span>{s.application?<small>{s.application.user.displayName}</small>:<button type="button" onClick={()=>remove(s.id)} aria-label="Supprimer le créneau"><X size={16} aria-hidden="true"/></button>}</div>)}</div></div>)}</div>}
      </div>
    </div>
  </div></section></Layout>;
}
