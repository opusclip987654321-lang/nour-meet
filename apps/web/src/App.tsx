...
const Avatar = ({ name, photoUrl, size, className }: { name?: string | null; photoUrl?: string | null; size?: "large"; className?: string }) => {
  const cls = `avatar${size ? ` ${size}` : ""}${className ? ` ${className}` : ""}`;
  return photoUrl ? <img className={cls} src={imgUrl(photoUrl)} alt="" /> : <div className={cls}>{(name ?? "?").slice(0, 2).toUpperCase()}</div>;
};
...
function ProfileEditor({onSaved}:{onSaved:()=>void}) {
  ...
  return <form className="panel form-grid" onSubmit={save}><div className="panel-title"><h2>Mon profil</h2><span>Informations privées</span></div>{message&&<Notice kind="success">{message}</Notice>}
    <div className="wide profile-photo-editor"><Avatar name={user?.displayName} photoUrl={user?.profile?.photoUrl} size="large"/><div>
