...
// soumise) : l'envoi est donc ouvert dès la demande (PENDING), pas seulement après approbation —
// jamais pour une demande refusée ou un compte suspendu. Débit limité : ces routes sont désormais
// accessibles à tout demandeur, pas seulement à un restaurateur approuvé par l'administration.
const PHOTO_RATE_LIMIT = { rateLimit: { max: smsVerification.mode === "mock" ? 1000 : 20, timeWindow: "10 minutes" } };
const galleryEditable = (status: string) => status === "PENDING" || status === "APPROVED";
app.post("/restaurants/me/photos", { preHandler: auth, config: PHOTO_RATE_LIMIT }, async (request, reply) => {
...
app.delete("/restaurants/me/photos/:photoId", { preHandler: auth, config: PHOTO_RATE_LIMIT }, async (request, reply) => {
  ...
  const restaurant = await prisma.restaurant.findUnique({ where: { ownerId: currentId(request) } });
  if (!restaurant || !galleryEditable(restaurant.status)) return reply.code(403).send({ error: "Aucun établissement modifiable pour ce compte" });
  const photo = await prisma.restaurantPhoto.findFirst({ where: { id: photoId, restaurantId: restaurant.id } });
  if (!photo) return reply.code(404).send({ error: "Photo introuvable" });
  // Le fichier est effacé avec sa ligne : sans cela, envoyer puis retirer en boucle remplissait le disque.
  await prisma.restaurantPhoto.delete({ where: { id: photo.id } });
  await deleteUploadedFile(photo.url, "/static/uploads/restaurants/");
